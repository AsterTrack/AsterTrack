...
if [[ $MODE = *"h"* ]]; then
	echo "Usage: sudo setup_image.sh [MODE] [IMAGE_FILE] [REPO_UPDATE] [SOURCE_PATH]
	MODE: [compile|dev-single|dev|wifi|minimal] - default: minimal
	...
	SOURCE_PATH: Optionally specify repository folder to look for camera source code - default: ../.. (git project structure)"
	exit 0
fi
...
# Dev image for headless (re-)compilation, will force a one-time-build by not writing prebuilt binaries
	FORCE_BUILD=True
