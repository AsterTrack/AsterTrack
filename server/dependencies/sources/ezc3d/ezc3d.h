...
public:
  std::shared_ptr<ezc3d::DataNS::Data> data; ///< Pointer that holds the data of the C3D
...
public:
  ...
  /// \return The parameters of the C3D
  ///
  EZC3D_API ezc3d::ParametersNS::Parameters &parameters();

  ///
  /// \brief Get all the frames from the data set
  /// \return The frames
  ///
  EZC3D_API std::vector<ezc3d::DataNS::Frame> &frames() const;
...
public:
  ...
  ///
  /// \brief Get the names of the poses in rotation data set
  /// \return The names of the poses in rotation data set
  ///
  EZC3D_API const std::vector<std::string> poseNames() const;

  ///
  /// \brief Get the index of a pose in the subframe
  /// \param poseName Name of the pose
  /// \return The index of the pose
  ///
  /// Search for the index of a pose into subframe by the name of this
  /// channel.
  ///
  /// Throw a std::invalid_argument if poseName is not found
  ///
  EZC3D_API size_t poseIdx(const std::string &poseName) const;
...
public:
  ...
  ///
  /// \brief Updates frame count optionally validates all frames to match the Parameters and Headers.
  ///
  ...
  ///
  EZC3D_API void finaliseFrames(bool validate = true);
  ...
  /// created
  ///
  EZC3D_API void addPoint(const std::string &pointName);
  ...
  /// created
  ///
  EZC3D_API void addPoint(const std::vector<std::string> &pointNames);
  ...
  /// is created
  ///
  EZC3D_API void addAnalog(const std::string &channelName);
  ...
  /// is created
  ///
  EZC3D_API void addAnalog(const std::vector<std::string> &channelName);

  ///
  /// \brief Create a labelled pose in rotation of the data set
  /// \param channelName The name of the pose to create
  ///
  /// Throw the same errors as updateParameter as it calls it after the pose
  /// is created
  ///
  EZC3D_API void addPose(const std::string &poseName);

  ///
  /// \brief Create labelled poses in rotation of the data set
  /// \param channelNames The name of the pose to create
  ///
  /// Throw the same errors as updateParameter as it calls it after the pose
  /// is created
  ///
  EZC3D_API void addPose(const std::vector<std::string> &poseName);
  ...
  void appendParameters(
      ...
      const std::vector<std::string> &newPoses = std::vector<std::string>());
