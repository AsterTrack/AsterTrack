...
int main(void)
{
	...
	uartState = UART_None;
	UARTSEL_CLAIM(UARTSEL_TX_PIN); // Take control of UART
	UARTSEL_CLAIM(UARTSEL_RX_PIN); // Prevent UART RX on SBC
	...
	UARTSEL_RELEASE(UARTSEL_TX_PIN);
	UARTSEL_RELEASE(UARTSEL_RX_PIN);
	...
	uint8_t syncPacket[NRF_SYNC_BROADCAST_LEN] = { 0x55, 0x77, 0x44, 0xCC };
	...
	while (1)
	{
		...
		if (piWantsBootloader)
		{
			...
			UARTSEL_CLAIM(UARTSEL_TX_PIN);
			UARTSEL_CLAIM(UARTSEL_RX_PIN);
			delayUS(100);
