...
#dtoverlay=ov9281,media-controller=0,clk-continuous=1
# Tried setting clk-continuous so default OV9282 driver allows setting HBLANK to 176 instead of 250 (HTS to 728 instead of 765 for 1280 width)
# While this worked, it also resulted in no frames sent by camera, as if HBLANK was just FORCED down to 176
...
# So instead, we build the old ov9281 driver and load it instead
...
if [[ -d "$STORAGE_PATH/drivers" ]]; then
	...
	echo "No camerqa drivers available, to build them on-device the camera needs internet to download the sources!"
