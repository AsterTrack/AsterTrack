...
void ezc3d::ParametersNS::Parameters::setMandatoryParameters() {
  // Mandatory groups
  {
    ...
    auto &used = grp.parameter("USED", ezc3d::DATA_TYPE::INT);
    if (used.valuesAsInt().empty()) used.set(0);

    grp.parameter("LABELS", ezc3d::DATA_TYPE::CHAR);
    grp.parameter("DESCRIPTIONS", ezc3d::DATA_TYPE::CHAR);
    grp.parameter("UNITS", ezc3d::DATA_TYPE::CHAR);

    auto &scale = grp.parameter("SCALE", ezc3d::DATA_TYPE::FLOAT);
    if (scale.valuesAsDouble().empty()) scale.set(-1.0);

    auto &rate = grp.parameter("RATE", ezc3d::DATA_TYPE::FLOAT);
    if (rate.valuesAsDouble().empty()) rate.set(0.0);

    auto &dataStart = grp.parameter("DATA_START", ezc3d::DATA_TYPE::INT);
    if (dataStart.valuesAsInt().empty()) dataStart.set(0);

    auto &frames = grp.parameter("FRAMES", ezc3d::DATA_TYPE::INT);
    if (frames.valuesAsInt().empty()) frames.set(0);
    else if (frames.valuesAsInt().size() > 1 || frames.valuesAsInt()[0] < 0)
    { // Make sure the value is actually a positive integer
        frames.set(static_cast<uint16_t>(frames.valuesAsInt()[0]));
    }
  }
  {
    if (!isGroup("ANALOG")) {
    ...
    auto &scale = grp.parameter("GEN_SCALE", ezc3d::DATA_TYPE::FLOAT);
    if (scale.valuesAsDouble().empty()) scale.set(1.0);

    grp.parameter("SCALE", ezc3d::DATA_TYPE::FLOAT);
    grp.parameter("OFFSET", ezc3d::DATA_TYPE::INT);
    grp.parameter("UNITS", ezc3d::DATA_TYPE::CHAR);

    auto &rate = grp.parameter("RATE", ezc3d::DATA_TYPE::FLOAT);
    if (rate.valuesAsDouble().empty()) rate.set(0.0);

    grp.parameter("FORMAT", ezc3d::DATA_TYPE::INT);
    grp.parameter("BITS", ezc3d::DATA_TYPE::INT);
  }
  {
    ...
    grp.parameter("TYPE", ezc3d::DATA_TYPE::INT);

    auto &zero = grp.parameter("ZERO", ezc3d::DATA_TYPE::INT);
    if (zero.valuesAsInt().size() != 2) zero.set(std::vector<int>() = {1, 0});

    grp.parameter("CORNERS", ezc3d::DATA_TYPE::FLOAT);
    grp.parameter("ORIGIN", ezc3d::DATA_TYPE::FLOAT);
    grp.parameter("CHANNEL", ezc3d::DATA_TYPE::INT);
    grp.parameter("CAL_MATRIX", ezc3d::DATA_TYPE::FLOAT);
  }
}
...
void ezc3d::ParametersNS::Parameters::setMandatoryParametersForSpecialGroup(
  ...
  if (!groupName.compare("ROTATION")) {
    ...
    auto &rate = grp.parameter("RATE", ezc3d::DATA_TYPE::FLOAT);
    if (rate.valuesAsDouble().empty()) rate.set(group("POINT").parameter("RATE").valuesAsDouble());

    auto &dataStart = grp.parameter("DATA_START", ezc3d::DATA_TYPE::INT);
    if (dataStart.valuesAsInt().empty()) dataStart.set(1);
  }
}
