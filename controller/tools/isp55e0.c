...
static const struct ch_profile profiles[] = {
	...
	},
	{
		...
		.family = 0x17,
		.type = 0x70,
		.code_flash_size = 196609, // Depends on configuration, this is minimum
		.mcu_id_len = 8,
		.need_last_write = true, // Verified needed for code size > 40KB or so
	},
	{}
};
...
static void read_config(struct device *dev)
{
	...
	char *str = malloc(sizeof(resp.config_data)*2 + 1);
	for (int i = 0; i < sizeof(resp.config_data); i++)
	{
		str[2*i+0] = hex[resp.config_data[i] >> 4];
...
static void load_file(struct device *dev, struct content *info)
{
	...
	printf("Firmware is %ld bytes rounted to %ld to write!\n", statbuf.st_size, info->len);
