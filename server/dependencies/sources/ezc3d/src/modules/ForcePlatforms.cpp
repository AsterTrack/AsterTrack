...
ezc3d::Modules::ForcePlatform::ForcePlatform(size_t idx,
                                             const ezc3d::c3d &c3d) {
...
void ezc3d::Modules::ForcePlatform::extractUnits(const ezc3d::c3d &c3d) {
...
size_t ezc3d::Modules::ForcePlatform::nbFrames() const { return _F.size(); }

size_t ezc3d::Modules::ForcePlatform::type() const { return _type; }
...
const ezc3d::Vector3d &ezc3d::Modules::ForcePlatform::meanCorners() const {
  return _meanCorners;
}
...
const std::vector<ezc3d::Vector3d> &ezc3d::Modules::ForcePlatform::Tz() const {
  return _Tz;
}

void ezc3d::Modules::ForcePlatform::extractType(size_t idx,
                                                const ezc3d::c3d &c3d) {
  const ezc3d::ParametersNS::GroupNS::Group &groupPF(
      c3d.parameters().group("FORCE_PLATFORM"));
  ...
  if (_type == 1) {
    ...
    if (c3d.parameters().group("FORCE_PLATFORM").isParameter("FPCOPPOLY")) {
      const auto &copPoly = c3d.parameters()
                                .group("FORCE_PLATFORM")
                                .parameter("FPCOPPOLY")
                                .valuesAsDouble();
      if (copPoly.size() != 0) {
        _type3copPoly = std::vector<double>(copPoly.begin() + idx * 12,
                                            copPoly.begin() + (idx + 1) * 12);
      }
    }
  } else if (_type == 5) {
...
ezc3d::Modules::ForcePlatforms::ForcePlatforms(const ezc3d::c3d &c3d) {
...
void ezc3d::Modules::ForcePlatform::extractCorners(size_t idx,
                                                   const ezc3d::c3d &c3d) {
  const ezc3d::ParametersNS::GroupNS::Group &groupPF(
      c3d.parameters().group("FORCE_PLATFORM"));
...
void ezc3d::Modules::ForcePlatform::extractOrigin(size_t idx,
                                                  const ezc3d::c3d &c3d) {
  const ezc3d::ParametersNS::GroupNS::Group &groupPF(
      c3d.parameters().group("FORCE_PLATFORM"));
...
void ezc3d::Modules::ForcePlatform::extractData(size_t idx,
                                                const ezc3d::c3d &c3d) {
  const ezc3d::ParametersNS::GroupNS::Group &groupPF(
      c3d.parameters().group("FORCE_PLATFORM"));
  ...
  const std::vector<size_t> &dimensions(
      groupPF.parameter("CHANNEL").dimension());
  ...
  for (const auto &frame : c3d.frames()) {
    for (size_t i = 0; i < frame.analogs->subframes.size(); ++i) {
      const auto &subframe(frame.analogs->subframes.at(i));
      if (_type == 1) {
        ...
        for (size_t j = 0; j < 3; ++j) {
          force_raw(j) = subframe.channels.at(channel_idx[j]).data;
          if (j < 2) {
            cop_raw(j) = subframe.channels.at(channel_idx[j + 3]).data;
          } else {
            tz_raw(j) = subframe.channels.at(channel_idx[j + 3]).data;
          }
        }
        ...
        if (_type == 3) {
          for (size_t j = 0; j < 8; ++j) {
            ch[j] = subframe.channels.at(channel_idx[j]).data;
          }
          // CalMatrix (the example I have does not have any)
          ...
          for (size_t j = 0; j < 3; ++j) {
            data_raw(j) = subframe.channels.at(channel_idx[j]).data;
            data_raw(j + 3) = subframe.channels.at(channel_idx[j + 3]).data;
          }
          if (_type == 4) {
        ...
        if (_type == 3) {
          ...
          // With th difference that offset are added after being computed,
          // which correspond to the actual documentation provided by Kistler,
          // as discussed in here: https://github.com/pyomeca/ezc3d/issues/253
          double xOffset =
              (_type3copPoly[0] * std::pow(CoP_raw.y(), 4) +
               _type3copPoly[1] * std::pow(CoP_raw.y(), 2) + _type3copPoly[2]) *
                  std::pow(CoP_raw.x(), 3) +
              (_type3copPoly[3] * std::pow(CoP_raw.y(), 4) +
               _type3copPoly[4] * std::pow(CoP_raw.y(), 2) + _type3copPoly[5]) *
                  CoP_raw.x();
          double yOffset =
              (_type3copPoly[6] * std::pow(CoP_raw.x(), 4) +
               _type3copPoly[7] * std::pow(CoP_raw.x(), 2) + _type3copPoly[8]) *
                  std::pow(CoP_raw.y(), 3) +
              (_type3copPoly[9] * std::pow(CoP_raw.x(), 4) +
               _type3copPoly[10] * std::pow(CoP_raw.x(), 2) +
               _type3copPoly[11]) *
                  CoP_raw.y();
          CoP_raw(0) -= xOffset;
          CoP_raw(1) -= yOffset;
...
ezc3d::Modules::ForcePlatforms::forcePlatform(size_t idx) const {
