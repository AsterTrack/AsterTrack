	...
	TargetTracking2DData &internalData)
{
	...
	omp_set_num_threads(std::min<int>(omp_get_max_threads(), params.maxParallelism));
	...
	TargetTracking2DData &internalData)
{
	...
	for (const Eigen::Isometry3f &pose : candidates)
	{
		...
		TargetMatchError errors = optimiseTargetPose<false>(calibs, points2D, targetMatch2D, target3D, pose, params.opt, track.filter.pose.cov);
