...
#define IM_TRUNC(_VAL)                  std::floor(_VAL)                                        // ImTrunc() is not inlined in MSVC debug builds
#define IM_ROUND(_VAL)                  std::round(_VAL)                                        //
...
static inline float  ImTrunc(float f)                                           { return (float)(long)(f); }
static inline ImVec2 ImTrunc(const ImVec2& v)                                   { return ImVec2(ImTrunc(v.x), ImTrunc(v.y)); }
static inline float  ImFloor(float f)                                           { return (float)((f >= 0 || (float)(long)f == f) ? (long)f : (long)f - 1); } // Decent replacement for floorf()
...
static inline int    ImToInt(float f)                                           { return f <= (float)INT_MIN? INT_MIN : (f >= (float)INT_MAX? INT_MAX : (int)f); }
...
struct ImGuiListClipperRange
{
    ...
    static ImGuiListClipperRange    FromPositions(float y1, float y2, int off_min, int off_max) { ImGuiListClipperRange r = { ImToInt(y1), ImToInt(y2), true, (ImS8)off_min, (ImS8)off_max }; return r; }
};
...
//#endif
    ...
    // Childs
    IMGUI_API bool          BeginChildEx(const char* name, ImGuiID id, const ImVec2& size_arg, ImGuiChildFlags child_flags, ImGuiWindowFlags window_flags, bool* p_open = NULL);
