	...
	else if (visCamera.visMode == VIS_BLOB)
	{
		...
		if (phase == PHASE_Automatic)
		{
			...
			for (auto &trackRecord : frame->trackers)
			{
				...
				if (trackRecord.id > 0)
					visualisePose(trackRecord.pose.filtered, Color{ 0.8, 0.8, 0, 1.0f }, 0.1f, 1.0f);
				// TODO: Track individual large markers (3/4) - visualisation
				/* else if (trackRecord.hasIMU)
					visualisePose(trackRecord.pose.filtered, Color{ 0.8, 0.8, 0, 1.0f }, 0.1f, 1.0f);
				else
					visualisePointsSpheres({ VisPoint{ trackRecord.pose.filtered.translation(), Color{ 0.8, 0.8, 0, 1.0f }, 10.0f } }); */
			}
			visSetupProjection(postProjMat);
		}
		else if (phase == PHASE_Tracking)
		{
			...
			for (auto &trackRecord : frame->trackers)
			{
				...
				{
					projectTarget(projected2D, tracker.calib, calib, trackRecord.ext->predicted, expandViewAngle);
				...
				{
					projectTarget(projected2D, tracker.calib, calib, trackRecord.pose.filtered, expandViewAngle);
