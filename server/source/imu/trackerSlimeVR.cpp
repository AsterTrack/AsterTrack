...
IMUDeviceProviderStatus SlimeVRReceiver::poll(int &updated, IMUDeviceList &removed, IMUDeviceList &added)
{
	...
	int reports = 0;
	int read;
	LOGC(LTrace, "Polling SlimeVR Receiver %s!", path.c_str());
	while ((read = hid_read(handle, data, sizeof(data))) > 0)
	{
		...
		{ // Multiple reports possible
			int reportID = data[0];
			LOGC(LTrace, "  SlimeVR Receiver Report %d sent %d bytes!", reportID, read);
			header = data+1;
		}

		uint8_t *packet = header+HID_HEADER_SIZE;
		int packets = (size-HID_HEADER_SIZE)/TRACKER_REPORT_SIZE;
		LOGC(LTrace, "  SlimeVR Receiver sent %d bytes (%d packets)!", size, packets);
		for (int p = 0; p < packets; p++)
		{
			int i = p*TRACKER_REPORT_SIZE;
			if (packet[i+0] == HEADER_SKIP) continue;
			uint8_t tracker_id = packet[i+1];
			LOGC(LTrace, "    Packet %d from SlimeVR Tracker IMU %d!", packet[i+0], tracker_id);
			...
			if (!devices[tracker_id])
			{
				...
				LOGC(LInfo, "    Registered new SlimeVR Tracker %d (%s)!", tracker_id, imu->id.serial().c_str());
	...
	if (read < 0)
	{
		LOGC(LWarn, "SlimeVR Receiver %s: %ls", path.c_str(), hid_error(handle));
...
bool SlimeVRReceiver::parseDataPacket(SlimeVRTracker &tracker, uint8_t data[TRACKER_REPORT_SIZE], TimePoint_t &timestamp)
{
	...
	switch (type)
	{
		...
		case TYPE_IMU_STATUS:
		{
			...
			tracker.temperature = (data[4] - 128.5f)/2 + 25;
			tracker.signalStrength = data[15]/255.0f;
			LOGC(LInfo, "    Temperature is %f", tracker.temperature);
...
std::string SlimeVRReceiver::getDescriptor()
{
	return asprintf_s("SlimeVR Receiver with %d known IMUs", (int)devices.size());
}

std::string SlimeVRTracker::getDescriptor()
{
	return asprintf_s("SlimeVR %s", id.string.c_str());
}
