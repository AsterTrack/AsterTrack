...
uint8_t ADDR_BROADCAST_META[] = { 0x23, 0x7E, 0xA5 };
uint8_t ADDR_BROADCAST_SYNC[] = { 0x25, 0x7E, 0xA5 };
uint8_t ADDR_BROADCAST_TRIG[] = { 0x27, 0x7E, 0xA5 }; // TODO: Use number of slots beyond that for trigger channels?
...
static void nrf_reset_state()
{ // Required when STM us reset/flashed while nRF is not
	// This requires a reset (esp. for RX) but nRF does not have a software reset mechanic

	// Disable RF
	GPIO_RESET(NRF_CTRL_GPIO_X, NRF_CTRL_CE_PIN);

	// Write config to power down
	uint8_t CONFIG = 0b00001000;
	spi_write_sync(0x00 | NRF_SPI_WRITE_REG, &CONFIG, 1);

	// Disable all data pipes
	uint8_t EN_RXADDR = 0b000000;
	spi_write_sync(0x02 | NRF_SPI_WRITE_REG, &EN_RXADDR, 1);

	// Clear status without handling it
	isSetup = true;
	uint8_t STATUS = NRF_STATUS_IRQ_MASK;
	spi_write_sync(0x07 | NRF_SPI_WRITE_REG, &STATUS, 1);
	isSetup = false;

	// Flush any past packets
	spi_write_sync(0x00 | NRF_SPI_FLUSH_TX, nullptr, 0);
	spi_write_sync(0x00 | NRF_SPI_FLUSH_RX, nullptr, 0);

	// PLOS_CNT is reset upon writing RF_CH
}
...
void nrf_setup_camera(uint8_t cameraAddress[3])
{
	...
	spi_write_sync(0x0C | NRF_SPI_WRITE_REG, RX_ADDR_P2, 1);
	uint8_t *RX_ADDR_P3 = ADDR_BROADCAST_TRIG;
	spi_write_sync(0x0D | NRF_SPI_WRITE_REG, RX_ADDR_P3, 1);
