...
ezc3d::ParametersNS::GroupNS::Parameter::Parameter(
    const std::string &name, const std::string &description,
    ezc3d::DATA_TYPE type, const std::vector<size_t> &dimension)
    : _name(name), _description(description), _isLocked(false),
      _data_type(type), _dimension(dimension), _isEmpty(true)
    {
...
void ezc3d::ParametersNS::GroupNS::Parameter::write(
  ...
  if (_data_type == DATA_TYPE::CHAR) {
    ...
    if (dimension.empty()) dimension.resize(1);
...
size_t ezc3d::ParametersNS::GroupNS::Parameter::longestElement() const {
  ...
  if (_dimension.size() == 1)
    ...
    for (size_t i = 0; i < _param_data_string.size(); ++i) {
      if (_param_data_string[i].size() > longest_str)
        longest_str = _param_data_string[i].size();
    }
    return longest_str;
  }
}
...
bool ezc3d::ParametersNS::GroupNS::Parameter::isDimensionConsistent(
  ...
  for (unsigned int i = 0; i < dimension.size(); ++i)
    dimProd *= dimension[i];
  if (dataSize == 0) {
    return dimension.size() == 0 || dimProd == 0;
  }
  return dataSize == dimProd;
}

void ezc3d::ParametersNS::GroupNS::Parameter::updateSingleDimension() {
  if (type() == DATA_TYPE::CHAR) {
    ...
    for (unsigned int i = 0; i < _param_data_string.size(); ++i)
      if (_param_data_string[i].size() > longest_str)
        longest_str = _param_data_string[i].size();
    _dimension = { longest_str, dataSize() };
  }
  else {
    _dimension = { dataSize() };
  }
...
size_t ezc3d::ParametersNS::GroupNS::Parameter::dataSize() const {
  switch (_data_type)
  {
  case DATA_TYPE::CHAR:
    return _param_data_string.size();
  case DATA_TYPE::BYTE:
  case DATA_TYPE::INT:
    return _param_data_int.size();
  case DATA_TYPE::FLOAT:
    return _param_data_double.size();
  default:
    return 0;
  }
}
...
void ezc3d::ParametersNS::GroupNS::Parameter::set(
  ...
  for (unsigned int i = 0; i < data.size(); ++i)
    if (data[i].size() > longest_str)
      longest_str = data[i].size();
  std::vector<size_t> dimensionWithStrLen = dimensionCopy;
  dimensionWithStrLen.insert(dimensionWithStrLen.begin(), longest_str);
...
std::vector<int> &
ezc3d::ParametersNS::GroupNS::Parameter::valuesAsByte() {
...
std::vector<int> &
ezc3d::ParametersNS::GroupNS::Parameter::valuesAsInt() {
...
std::vector<double> &
ezc3d::ParametersNS::GroupNS::Parameter::valuesAsDouble() {
...
std::vector<std::string> &ezc3d::ParametersNS::GroupNS::Parameter::valuesAsString() {
