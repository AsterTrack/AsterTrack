...
// May need to add this to project-wide defines
...
auto lock(SynchronizedLocker... lockersIn)
  ...
  auto indices = std::index_sequence_for<SynchronizedLocker...>{};

  int indexLocked = -1;
  while (true) {
    bool couldLockAll = true;
    int index = 0;

    [&] <std::size_t... I> (std::index_sequence<I...>)
    { // Templated lambda is a C++20 feature, would have to be a separate function for pre-C++20
      // See: https://www.fluentcpp.com/2021/03/05/stdindex_sequence-and-its-improvement-in-c20/
      ([&]
      {
        if (!couldLockAll) return; // Break out of the loop by skipping rest
        if (I == indexLocked) return;
        // if we should try_lock on the current locker then do so
        auto lockedPtr = std::get<I>(lockers).tryLock();
        if (lockedPtr) {
          // store the locked mutex in the list we return
          std::get<I>(lockedPtrs) = std::move(lockedPtr);
          return;
        }
