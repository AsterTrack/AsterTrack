...
ezc3d::ParametersNS::GroupNS::Parameter &ezc3d::ParametersNS::GroupNS::Group::parameter(
  const std::string &name, ezc3d::DATA_TYPE type, const std::vector<size_t> &dimension) {
  if (type == ezc3d::DATA_TYPE::NO_DATA_TYPE)
    throw std::runtime_error("Data type is not set");

  size_t alreadyExistIdx(SIZE_MAX);
  for (size_t i = 0; i < _parameters.size(); ++i)
    if (!parameter(i).name().compare(name)) {
      alreadyExistIdx = i;
      break;
    }
  if (alreadyExistIdx == SIZE_MAX)
  {
    return _parameters.emplace_back(name, "", type, dimension);
  }
  if (type != _parameters[alreadyExistIdx].type())
    throw std::runtime_error("Data type mismatching");
  if (!_parameters[alreadyExistIdx].isDimensionConsistent(_parameters[alreadyExistIdx].dataSize(), dimension))
    throw std::runtime_error("Data dimension mismatching");
  return _parameters[alreadyExistIdx];
}
